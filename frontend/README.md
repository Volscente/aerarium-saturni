...
- **`app/(tabularium)/tabularium/etf-actions.ts`** — `createEtf`, `updateEtf`, `deleteEtf`, `addPriceSnapshot`, `updatePriceSnapshot`, `deletePriceSnapshot`, `fetchPriceHistory` Server Actions; parse JSONB string fields to `Record<string, number>` before backend call; each write action calls `revalidateTag('etfs')`, `revalidateTag('portfolio-overview')`, and `revalidateTag('holdings-exposure')` on success; return `{ success: true } | { error: string }` (write actions) or `EtfPriceEntry[] | { error: string }` (`fetchPriceHistory`); exports `EtfPriceEntry` interface
...
- **`app/(tabularium)/tabularium/actions.ts`** — `createTransaction`, `updateTransaction`, `deleteTransaction` Server Actions: each re-validates with Zod (create/update) or calls the backend directly (delete), then calls `revalidateTag('transactions')`, `revalidateTag('portfolio-overview')`, and `revalidateTag('holdings-exposure')`; returns `{ success: true } | { error: string }`
