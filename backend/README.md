...
- **`src/backend/models.py`** — `Base` (declarative base); `Transaction` ORM class (14 columns); `Etf` ORM class (25 columns including four JSONB distribution columns — `geographical_distribution`/`sector_distribution` default to `{}` via Python-side `default=dict`, since they are derived from holdings uploads rather than user-supplied at creation); `EtfHolding` child class (9 columns: `id`, `etf_id` FK cascade, `stock_isin` nullable, `stock_ticker` nullable, `stock_country` nullable (ISO 3166-1 alpha-2), `stock_sector` nullable (canonical GICS-like bucket name), `stock_name`, `weight_percentage`, `snapshot_date`; at least one of `stock_isin`/`stock_ticker` required per row; composite index on `(etf_id, snapshot_date)`); `EtfPriceHistory` child class with FK cascade and composite index
...
- **`backend/alembic/versions/005_add_etf_holdings_stock_sector.py`** — Fifth migration: adds `stock_sector VARCHAR(50)` nullable to `etf_holdings`, storing each holding's canonical GICS-like sector bucket
...
- **`src/backend/schemas/etfs.py`** — `EtfCreate` (ISIN `field_validator`; `model_validator` requiring bond distribution maps when `asset_class = Bonds`; does **not** accept `geographical_distribution`/`sector_distribution` — those are derived server-side from holdings uploads, never user-supplied); `EtfUpdate` (all fields optional for partial updates, same exclusion); `EtfResponse` (ORM-mode; still returns `geographical_distribution`/`sector_distribution` as read-only fields); `EtfPriceCreate`; `EtfPriceResponse`; `EtfHoldingRow` (CSV row parsing: `stock_isin` nullable + `stock_ticker` nullable with `model_validator` requiring at least one; ISIN validator normalises to uppercase, accepts blank/empty as absent)
...
- **`src/backend/routers/etfs.py`** — Seven FastAPI route handlers for ETF CRUD, price history retrieval, manual price logging, and atomic CSV/XLSX holdings upload; uses `Depends(get_session)` and `python-multipart` `UploadFile` for file handling. `create_etf` explicitly defaults `geographical_distribution`/`sector_distribution` to `{}` when constructing `Etf(...)`, since `EtfCreate` no longer carries those fields. `upload_holdings` computes both fields fresh on every upload via `_aggregate_fund_distribution(holdings, key)` — sums each new holding's `weight_percentage` by `stock_country`/`stock_sector` directly across just this ETF's own rows (not portfolio-weighted, unlike the look-through aggregation in `routers/portfolio.py`); a row whose country/sector couldn't be derived (or a CSV with no such column) is bucketed under the literal `"Unknown"` key rather than a null one. The distribution update happens in the same transaction as the holdings replace, before the single `session.commit()`. `upload_holdings` also calls `resolve_stock_isin_aliases` after committing the new holdings; a failure there (e.g. OpenFIGI unreachable) is caught and non-fatal — the holdings are already committed, and reconciliation is simply retried on the next upload of any ETF.
- **`src/backend/converters/holdings_xlsx.py`** — `convert_holdings_xlsx(source, ticker=None)`: converts an issuer holdings XLSX export into `EtfHoldingRow`-shaped dict rows; called directly from `upload_holdings` when the uploaded filename ends in `.xlsx`. `source` is a `Path` or an open binary file-like object (e.g. the in-memory uploaded bytes); `ticker` defaults to `source.stem` for a `Path` and is required otherwise. The ticker is resolved from its *leading* alphanumeric token rather than an exact match (`ISSUER_BY_TICKER`: `EUNL`→iShares, `VWCE`→Vanguard, `LYP6`→Amundi), since browsers suffix repeat downloads of the same file (`EUNL (1).xlsx`, `EUNL_2026-07-23.xlsx`) and an exact-stem match would 422 on routine re-uploads. Each per-issuer parser also differs in layout, language, and available identifier (ticker vs ISIN); drops cash/money-market/derivative and zero-weight rows; tolerates Amundi's malformed `styles.xml` by rewriting invalid aRGB color values in memory before retrying. Each row also carries `stock_country` (ISO 3166-1 alpha-2): free from the ISIN's own first two characters for Amundi, translated from the German `Standort` column via the static `GERMAN_COUNTRY_TO_ISO` dict for iShares, taken directly from the already-ISO `Region` column for Vanguard. Each row also carries `stock_sector`, normalised via `_normalize_sector(raw, issuer)` into one canonical ~11-bucket GICS-like taxonomy (e.g. `"Information Technology"`, `"Financials"`): iShares and Amundi both publish German labels but with different phrasing for the same concepts, so both resolve through the shared `SECTOR_TO_CANONICAL_DE` dict, while Vanguard's English labels resolve through the separate `SECTOR_TO_CANONICAL_EN` dict; an unmapped raw label returns `None` rather than raising, mirroring `_german_country_to_iso`'s style. `write_csv(rows, path)` is a standalone helper (used in tests) for writing converted rows to an inspectable CSV file — not on the upload request path. `resolve_stock_isin_aliases(session, http_client)` backfills `stock_isin` on ticker-only rows: OpenFIGI's `/v3/mapping` only resolves ISIN → ticker (never the reverse), so it resolves every distinct ISIN already in `etf_holdings` to its ticker and updates any row still missing `stock_isin` whose `(stock_ticker, stock_country)` matches — country as well as ticker avoids merging unrelated companies that coincidentally share a ticker on different exchanges.
- **`src/backend/schemas/portfolio.py`** — `PortfolioRowResponse` (six fields: owner, broker_platform, total_invested, current_value, performance_abs, performance_pct — performance fields nullable when price data absent); `PortfolioOverviewResponse` wrapping a list of rows; `HoldingContribution` (per-ETF contribution to a stock's look-through weight, including `etf_portfolio_weight_percentage`/`stock_weight_in_etf_percentage` alongside the computed `contribution_weight_percentage`); `HoldingExposureResponse` (per-stock look-through exposure with nested `contributions`); `RiskAlert` (`rule: Literal["concentration_risk", "data_freshness_risk"]`, a ready-to-render `message`, plus rule-specific context fields); `HoldingsExposureResponse` wrapping a list of holdings, `skipped_etfs`, and `alerts`; `BucketEtfContribution` (an ETF's combined contribution to a geography/sector bucket — `etf_ticker`/`etf_name`/`etf_portfolio_weight_percentage`/`bucket_weight_in_etf_percentage`/`contribution_weight_percentage`/`snapshot_date`, i.e. the same shape as `HoldingContribution` but for a bucket instead of a single stock); `CountryExposureResponse`/`HoldingsGeographyResponse` and `SectorExposureResponse`/`HoldingsSectorsResponse` (bucketed by `country_code`/`sector`, either possibly `None`, each carrying a `contributions: list[BucketEtfContribution]` — one row per contributing ETF, not per stock)
- **`src/backend/routers/portfolio.py`** — `GET /portfolio/overview` handler; two-phase SQLAlchemy async query (CTE for net holdings per ISIN, correlated subquery for latest price); Python-side grouping and null propagation; `_build_portfolio_query` and `_to_row_response` helpers. `_build_holdings_exposure_query` joins each owned ETF's current value (same net-holdings + latest-price pattern, grouped by ISIN alone rather than per owner/broker) against its latest `EtfHolding` snapshot, now also selecting `stock_country`/`stock_sector`. `_aggregate_stock_groups(session)` runs that query once and groups rows by `COALESCE(stock_isin, stock_ticker)` — computing each holding's weighted contribution to total portfolio value, capturing each stock's `stock_country`/`stock_sector` from the first row seen for that stock (treated as invariant across the ETFs holding it) — shared by all three `GET /holdings/*` handlers below so the query only runs once per request regardless of which is called. `_bucket_etf_contributions(groups, bucket_field)` is a second, small Python-side pass re-grouping each per-stock group's own per-ETF `contributions` list by `("stock_country" | "stock_sector", etf_ticker)` — no additional query — so an ETF holding several stocks within the same bucket collapses into ONE combined `BucketEtfContribution` row (summing `stock_weight_in_etf_percentage`/`contribution_weight_percentage` across those stocks) rather than one row per stock; a stock with no derivable country/sector buckets under `None`. `GET /portfolio/holdings/exposure` (`get_holdings_exposure`) builds its response from `_aggregate_stock_groups`'s groups directly (stock_country/stock_sector stay internal, not exposed on `HoldingExposureResponse`); `GET /portfolio/holdings/geography` (`get_holdings_geography`) and `GET /portfolio/holdings/sectors` (`get_holdings_sectors`) each bucket those same groups via `_bucket_etf_contributions`. An ETF with no price record is excluded from the aggregation and reported in `skipped_etfs` rather than nulling the whole response — shared across all three endpoints. `CONCENTRATION_THRESHOLD_PCT`/`FRESHNESS_THRESHOLD_DAYS` module constants; `_build_concentration_alerts` (one `RiskAlert` per holding above 10%) and `_build_freshness_alerts` (one `RiskAlert` per distinct owned ETF whose latest `snapshot_date` is more than 60 days old) are concatenated into `GET /holdings/exposure`'s `alerts` field only
...
- `GET /portfolio/holdings/exposure` — Aggregate look-through single-stock exposure across all owned ETFs; returns `HoldingsExposureResponse` with one `HoldingExposureResponse` per distinct stock identity (ordered by `total_weight_percentage` descending), each carrying a `contributions` breakdown per source ETF; `skipped_etfs` lists tickers excluded from the aggregation for lacking a price record; `alerts` lists active Concentration Risk (`total_weight_percentage` > 10%) and Data Freshness Risk (an owned ETF's holdings `snapshot_date` more than 60 days old) warnings, recomputed fresh on every request
- `GET /portfolio/holdings/geography` — Aggregate the same look-through stock exposure into per-country buckets; returns `HoldingsGeographyResponse` with one `CountryExposureResponse` per distinct `stock_country` value (ISO 3166-1 alpha-2, ordered by `total_weight_percentage` descending), each carrying a `contributions` list of the contributing ETFs (ticker/name/portfolio share/fund weight/contribution/as-of, sorted DESC by contribution — one row per ETF, not per stock; an ETF holding multiple stocks in the bucket is combined into a single row); a stock whose country couldn't be derived buckets under `country_code: null`; `skipped_etfs` mirrors `/holdings/exposure`
- `GET /portfolio/holdings/sectors` — Same shape as `/holdings/geography`, bucketed by `stock_sector` (the canonical GICS-like name normalised at holdings-upload time) instead of country; returns `HoldingsSectorsResponse`; a stock whose sector couldn't be normalised buckets under `sector: null`
...
- `src/backend/schemas/portfolio.py` — Replaced `BucketStockContribution` with `BucketEtfContribution` (`etf_ticker`, `etf_name`, `etf_portfolio_weight_percentage`, `bucket_weight_in_etf_percentage`, `contribution_weight_percentage`, `snapshot_date` — the same shape as `HoldingContribution`, but for an ETF's combined contribution to a bucket rather than one ETF's contribution to a single stock). `CountryExposureResponse.holdings`/`SectorExposureResponse.holdings` renamed to `contributions: list[BucketEtfContribution]`.
- `src/backend/routers/portfolio.py` — Replaced `_bucket_stock_groups` with `_bucket_etf_contributions`: instead of listing the individual stocks in a bucket, it re-groups each per-stock group's own per-ETF `contributions` (already computed by `_aggregate_stock_groups`) by `(bucket, etf_ticker)` — summing `stock_weight_in_etf_percentage`/`contribution_weight_percentage` across every stock that ETF holds within the bucket. An ETF holding multiple stocks in the same country/sector now collapses into ONE row instead of appearing once per stock, mirroring the Holdings tab's own per-ETF breakdown at the bucket level instead of the stock level. `get_holdings_geography`/`get_holdings_sectors` updated to call the renamed helper and build `contributions=bucket["contributions"]`.
- `tests/conftest.py` — `mock_session_geography_multi_country` gained a second US/Information-Technology stock (AAPL) held by the same ETF (EUNL) as the existing NVDA row, specifically to exercise the new same-ETF-merge behavior.
- `tests/routers/test_portfolio.py` — Rewrote the geography/sector bucket tests around `contributions` instead of `holdings`; added `test_get_holdings_geography_merges_same_etf_across_stocks_in_bucket` and its sector equivalent, asserting one EUNL row combining NVDA's and AAPL's fund weight/contribution rather than two.
- Verified end-to-end: `GET /portfolio/holdings/geography`'s "US" bucket returns exactly 2 contributions (VWCE, EUNL) — not 500+ individual stock rows — each with the same Portfolio Share/Fund Weight/Contribution/As-of shape already used by the Holdings tab; confirmed live in the browser for both Geography and Sectors tabs.

#### 2026-09-16 (v0.5.4)

- `src/backend/schemas/etfs.py` — Removed `geographical_distribution`/`sector_distribution` from `EtfCreate` and `EtfUpdate`: both are now derived from an ETF's own holdings uploads, not user-supplied. `EtfResponse` is unchanged — both fields are still returned, read-only.
- `src/backend/models.py` — `Etf.geographical_distribution`/`sector_distribution` columns gain a Python-side `default=dict`, as a defensive fallback for any future insertion path that doesn't explicitly set them (the primary path, `create_etf`, sets them explicitly — see below — since ORM column defaults only apply at flush time and this router's tests use a mocked session with no real flush).
- `src/backend/routers/etfs.py` — `create_etf` now explicitly constructs `Etf(geographical_distribution={}, sector_distribution={}, **body.model_dump())`, since the request body no longer carries those fields. New `_aggregate_fund_distribution(holdings, key)` helper: sums each new holding's `weight_percentage` by `stock_country`/`stock_sector` directly across a single ETF's own upload (not portfolio-weighted — a different aggregation from the look-through one in `routers/portfolio.py`); a holding with no derivable country/sector (or a CSV missing that column) is bucketed under `"Unknown"`. `upload_holdings` now calls this twice (once per field) and sets `etf.geographical_distribution`/`etf.sector_distribution` before the existing atomic delete-then-insert commit, so every holdings upload (CSV or issuer XLSX) keeps the ETF's own fund-level distribution in sync with its actual holdings — replacing whatever was there before (including anything previously entered manually via the now-removed form fields).
- `tests/conftest.py` — `VALID_ETF_PAYLOAD` no longer includes `geographical_distribution`/`sector_distribution`.
- `tests/routers/test_etfs.py` — New tests: a freshly created ETF defaults both fields to `{}`; supplying them in the create payload is silently ignored; `_aggregate_fund_distribution` unit tests (sums by bucket, `None`/absent bucketed as `"Unknown"`, empty holdings → `{}`); `upload_holdings` populates both fields correctly from a CSV that carries `stock_country`/`stock_sector` columns, and buckets everything under `"Unknown"` for a CSV that doesn't.
- Verified end-to-end: re-uploading the real `EUNL.xlsx` fixture for an ETF that previously had manually-entered (and by then stale) `geographical_distribution`/`sector_distribution` correctly replaced both with the real computed values (11 canonical sectors, real ISO country codes); creating an ETF while supplying both fields in the request body confirmed they're ignored (`{}` in the response); editing an ETF's other fields afterward left the holdings-derived distribution untouched.

#### 2026-09-16 (v0.5.3)

- `backend/alembic/versions/005_add_etf_holdings_stock_sector.py` — New migration: adds nullable `stock_sector VARCHAR(50)` to `etf_holdings`.
- `src/backend/models.py` — `EtfHolding.stock_sector: Mapped[str | None]` added, directly after `stock_country`.
- `src/backend/converters/holdings_xlsx.py` — New `SECTOR_TO_CANONICAL_DE`/`SECTOR_TO_CANONICAL_EN` static dicts and `_normalize_sector(raw, issuer)` helper, mirroring `_german_country_to_iso`'s style (thin `.get()`, returns `None` on an unmapped miss rather than raising). All three converters now read their previously-discarded sector column (`_convert_ishares`: 3rd positional element, was `_sector`; `_convert_vanguard`: 4th positional element, was `_sector`; `_convert_amundi`: `row[6]`, never read before) and add a normalised `stock_sector` key to each returned row dict, following the exact same out-of-band path as `stock_country` (not a field on `EtfHoldingRow`). iShares and Amundi both publish German sector labels but with different phrasing for the same concepts (verified against the real `EUNL.xlsx`/`LYP6.xlsx` fixtures — 11 distinct raw values each, only 4 phrasings coincide exactly), so both resolve through the shared `SECTOR_TO_CANONICAL_DE` dict; Vanguard's English labels resolve through `SECTOR_TO_CANONICAL_EN`.
- `src/backend/routers/etfs.py` — `upload_holdings` now also passes `stock_sector=row_dict.get("stock_sector")` when constructing `EtfHolding`, alongside the existing `stock_country` line.
- `src/backend/schemas/portfolio.py` — New `BucketStockContribution`, `CountryExposureResponse`, `HoldingsGeographyResponse`, `SectorExposureResponse`, `HoldingsSectorsResponse` Pydantic v2 schemas.
- `src/backend/routers/portfolio.py` — `_build_holdings_exposure_query`'s select extended with `EtfHolding.stock_country`/`stock_sector` (no CTE/join changes). The row-processing logic previously inline in `get_holdings_exposure` is now the shared `_aggregate_stock_groups(session)` helper (each stock's `stock_country`/`stock_sector` captured from the first row seen for that stock, treated as invariant across the ETFs holding it), plus a new `_bucket_stock_groups(groups, bucket_field)` second-pass helper. Two new endpoints, `GET /portfolio/holdings/geography` and `GET /portfolio/holdings/sectors`, reuse `_aggregate_stock_groups`'s per-stock aggregation (no second query) and bucket it by country/sector respectively; `get_holdings_exposure`'s own response shape and behavior are unchanged. This design avoids double-counting a stock across the multiple ETFs that hold it, since geography/sector buckets are built from the already-deduplicated per-stock totals rather than re-aggregating from raw rows.
- `tests/conftest.py` — Fixed a pre-existing gap: `_make_holdings_exposure_row()` never set `stock_country`/`stock_sector`, defaulting to a truthy `MagicMock` rather than `None` on any test that forgot an explicit override; now defaults both to `None`. New `mock_session_geography_multi_country` fixture and `client_geography_*`/`client_sectors_*` fixtures (via a new small `_make_bucket_client` factoring of the repeated `TestClient`/engine-patch boilerplate, introduced only for these new fixtures).
- `tests/converters/test_holdings_xlsx.py` — Added `stock_sector` to the three existing per-issuer key-set/exact-row assertions (verified against the real fixtures: NVDA/NVDA/ASML all normalise to `"Information Technology"`); 3 new unit tests for `_normalize_sector` (German merged-dict resolution across both issuers without collision, Vanguard's English dict, and an unmapped/absent value returning `None`).
- `tests/routers/test_portfolio.py` — 8 new tests for `GET /portfolio/holdings/geography` and `GET /portfolio/holdings/sectors`: empty case, correct bucketing (including the `null`/unmapped bucket) with weighted totals, DESC ordering, no per-ETF fields leaking into a bucket's `holdings`, and `skipped_etfs` populating for a priceless ETF.
- Verified end-to-end against the real `EUNL.xlsx`/`VWCE.xlsx`/`LYP6.xlsx` fixtures re-uploaded through `POST /etfs/{id}/holdings/upload`: `/holdings/sectors` returns exactly 11 canonical buckets with zero unmapped entries; `/holdings/geography` returns 49 country buckets; `/holdings/exposure`'s response shape is unchanged after the `_aggregate_stock_groups` refactor.

This implements the backend half of the **Portfolio UI Restructuring: Sub-Tab Architecture** initiative's Geography/Sectors tabs (initiative 15).

#### 2026-09-01 (v0.5.1)

- `src/backend/schemas/portfolio.py` — New `RiskAlert` Pydantic v2 schema (`rule: Literal["concentration_risk", "data_freshness_risk"]`, a ready-to-render `message`, plus rule-specific context fields); `HoldingsExposureResponse` gains `alerts: list[RiskAlert]`.
- `src/backend/routers/portfolio.py` — New `CONCENTRATION_THRESHOLD_PCT = 10` and `FRESHNESS_THRESHOLD_DAYS = 60` module constants; `_build_concentration_alerts()` emits one `RiskAlert` per holding whose `total_weight_percentage` exceeds the threshold (strict `>`); `_build_freshness_alerts()` emits one `RiskAlert` per distinct owned ETF whose latest holdings `snapshot_date` is more than 60 days old (strict `>`), reusing the existing per-`etf_id` dedup dict (now also carrying each ETF's own `snapshot_date` and `etf_name`) rather than a second query. `get_holdings_exposure` concatenates both into the response's new `alerts` field — no new query, both rules derived entirely from data already computed for `holdings`/`skipped_etfs`.
- `tests/routers/test_portfolio.py` — 6 new tests for `GET /portfolio/holdings/exposure`'s `alerts` field: no alerts when nothing breaches, concentration alert above/at-exactly the 10% threshold, freshness alert above/at-exactly the 60-day threshold, and both alert types firing together for one stock inside one concentrated-and-stale ETF.
- `tests/conftest.py` — New `_make_holdings_exposure_row`-backed fixtures for the 6 new tests; the existing `mock_session_exposure_merged_by_isin` fixture's two rows now pass an explicit `snapshot_date=date.today()` (previously relying on the row-builder's hardcoded `2026-07-23` default) so its reused "no alerts" test stays correct indefinitely rather than starting to fail once that hardcoded date passes 60 days old.

This is TASK-2 of the **Holdings Risk Alerts** initiative (see `.claude/14-holdings-risk-alerts/rfc.md`).
