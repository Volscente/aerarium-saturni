...
- **`app/(tabularium)/tabularium/portfolio/components/PortfolioPageClient.tsx`** — `'use client'` page container; renders (in order) `<PortfolioOverviewTable>`, a `skipped_etfs` advisory line when non-empty, `<RiskAlertPanel>`, `<ConcentrationAlertBadge>`, `<HoldingsBarChart>`, `<HoldingsTreemap>`, then `<HoldingsExposureTable>`; exports `PortfolioRowResponse`, `PortfolioOverviewResponse`, `HoldingContribution`, `HoldingExposureResponse`, `RiskAlert`, and `HoldingsExposureResponse` TypeScript interfaces (all but the first two mirror `backend/src/backend/schemas/portfolio.py`)
