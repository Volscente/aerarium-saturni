...
- `GET /portfolio/holdings/geography` — Aggregate the same look-through stock exposure into per-country buckets; returns `HoldingsGeographyResponse` with one `CountryExposureResponse` per distinct `stock_country` value (ISO 3166-1 alpha-2, ordered by `total_weight_percentage` descending), each carrying a `holdings` list of the contributing stocks (ticker/ISIN/name/weight, sorted DESC); a stock whose country couldn't be derived buckets under `country_code: null`; `skipped_etfs` mirrors `/holdings/exposure`
